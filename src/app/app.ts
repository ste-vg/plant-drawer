...
export class App
{
	...
	private startBranch(sections:number, position:Position, setColors:boolean = false)
	{
		...
		let newBranch = new Branch(this.svg, settings, this.grid/2 + Math.random() * this.grid/2, this.branches.length > 0 ? this.branches[Math.floor(Math.random() * (this.branches.length - 1))]: null);
